...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope face-recognition-cli
...
scope:** the default personal scope is **private** (`--scope face-recognition-cli
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `face-recognition-cli`). Private records are served only to a recall in the same
