...
**Provenance.** Extracted from ``reachy-mini-cli``'s
``reachy/vision/face_store.py`` (issue #1), which was itself cited from
``reachy_nova.face_manager.FaceManager`` (cite-don't-import). The port is
faithful: the cosine-similarity matcher, the temporary-vs-permanent tier split,
the 4-char id alphabet, the match threshold, the TTL, and above all the on-disk
layout (one JSON index + one ``.npy`` file per stored embedding) are byte-
compatible with what reachy writes today. That compatibility is the whole
reason the engine was extracted rather than swapped for dlib: SFace embeddings
and dlib encodings are both 128-dim but live in *different vector spaces*, so
changing either the model or the layout would invalidate every face already
enrolled on a robot and force real people to re-enrol.
``tests/fixtures/reachy_store/`` is a tree written by reachy's own
``FaceStore.enroll()`` and is loaded through this module by the test suite to
keep that claim honest.

The **one deliberate deviation from reachy** is the storage root.
:func:`default_base_dir` no longer resolves to ``reachy.daemon.state_dir() /
"faces"`` — this package owns its own state dir and partitions it into
per-consumer banks, so the default is
:func:`face_recognition_cli.state.bank_dir`, i.e.
``<state dir>/banks/<bank>`` (``banks/default`` unless ``$FACE_RECOGNITION_BANK``
or an explicit bank says otherwise). reachy keeps its already-enrolled faces by
pointing ``base_dir=`` at its existing store; nothing about the format changes,
only where an *unconfigured* store looks by default. This module imports nothing
from ``reachy.*``.

Deviations inherited from the reachy port (relative to nova), kept verbatim:
...
* **No admin-auth tier.** Nova's ``FaceManager`` bakes in a hard-coded admin
  ...
  feature here either, so all of that (``ADMIN_NAME``, ``_ensure_admin``,
  ``is_admin``) stays dropped — explicitly out of scope.
* **``enroll(name, embedding)`` replaces ``consent(temp_id, name)``.** Nova's
  ...
  temporary tier. This store keeps the temporary tier (useful for a "who are
  you?" enrollment flow) but *also* exposes a direct ``enroll`` that creates a
  permanent record from a fresh embedding in one call.
* **No ``merge`` / ``add_angles`` / ``get_person_images``.** Nova's multi-angle
  enrollment and face-merge admin tools aren't part of this contract; the
  on-disk shape (a list of ``embedding_files`` per face) leaves room to add
  them later without a migration.
* **Corrupt/missing index degrades to "start fresh", never raises** — rather
  than nova's bare ``json.load`` (which nova wraps in its own try/except at the
...
flow (see a face, stash it under a TTL, name it later), and dropping it would
mean re-deriving it when that flow lands; carrying ~30 lines of dead-but-tested
surface is the cheaper side of that trade.

Determinism seams for tests: every time-sensitive method takes an optional
``now=`` override; the constructor takes ``base_dir=`` to point at an isolated
directory instead of the real state dir, and ``clock=`` for the default-now
callable.
"""
...
#: Matches nova's ``FaceManager.match`` default, carried through reachy.
...
def default_base_dir() -> Path:
    """The default persistence root: the current bank, ``<state>/banks/<bank>``.

    The one deliberate deviation from reachy's ``<state_dir>/faces`` — see the
    module docstring. Bank selection is :func:`face_recognition_cli.state.
    resolve_bank`'s job; this store treats whatever directory it is handed as a
...
class FaceStore:
    ...
    base_dir:
        ...
        Tests pass an isolated ``tmp_path``; reachy-mini-cli points this at its
        own pre-existing ``<state dir>/faces`` tree so enrolled faces keep
