...
face-recognition-cli vendors its `.claude/skills/` from **guildmaster** — the
...
Every vendored `SKILL.md` carries `type: command`. face-recognition-cli
...
#   - consumer-identifying prose: `guildmaster` → `face-recognition-cli` (NOT
...
If a re-sync would lose a face-recognition-cli adaptation, lift the change
...
#     → `which face-recognition-cli vendors from guildmaster`
